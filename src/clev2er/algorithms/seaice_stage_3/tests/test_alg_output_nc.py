...
clev2er.algorithms.seaice_stage_3.alg_output_nc.py
"""
...
from clev2er.algorithms.seaice_stage_3.alg_output_nc import Algorithm
...
def test_output_nc(
    previous_steps: Dict,
    thisalg: Algorithm,
    tmp_path,  # pylint: disable=redefined-outer-name
) -> None:
    ...
    test that a nc file is created in a temporary directory when ran
    """
    ...
    thisalg.output_directory = Path(str(tmp_path))
    ...
    # Algorithm tests
    output_files = os.listdir(str(tmp_path / "2022"))
    print(output_files)

    assert len(fnmatch.filter(output_files, "*.nc")) >= 1, ".nc file wasn't created"
